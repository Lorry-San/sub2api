...
func defaultModelsListCandidateIDs(platform string) []string {
	...
	case PlatformKiro:
		return []string{
			DefaultKiroModelSonnet,
			DefaultKiroModelHaiku,
			DefaultKiroModelOpus,
			"claude-sonnet-4",
			"claude-haiku-4.5",
			"claude-opus-4.5",
		}
...
func (s *adminServiceImpl) CreateGroup(ctx context.Context, input *CreateGroupInput) (*Group, error) {
	...
	if group.RequireOAuthOnly && (group.Platform == PlatformOpenAI || group.Platform == PlatformAntigravity || group.Platform == PlatformKiro || group.Platform == PlatformAnthropic || group.Platform == PlatformGemini) && len(accountIDsToCopy) > 0 {
